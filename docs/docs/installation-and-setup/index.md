...
**EasyTemplates** is a cross-platform Python library compatible with
**Python 3.11** through **3.13**.

To install and set up EasyTemplates, we recommend using
...
This section describes the simplest way to set up EasyTemplates using
**Pixi**.
...
#### Setting up EasyTemplates with Pixi
...
- Set the Python version for the Pixi environment (e.g., 3.13):
  ...
  ```
- Add EasyTemplates to the Pixi environment from PyPI:
  ```txt
  pixi add --pypi easytemplates
  ```
- Add a Pixi task to run EasyTemplates commands easily:
  ```txt
  pixi task add easytemplates "python -m easytemplates"
  ```

#### Updating Pixi and EasyTemplates

- To update all packages in the Pixi environment, including EasyTemplates:
  ```txt
  pixi update
  ```
...
This section describes how to install EasyTemplates using the traditional
...
EasyTemplates is available on **PyPI (Python Package Index)** and can be
installed using `pip`. To do so, use the following command:

```txt
pip install easytemplates
```

To install a specific version of EasyTemplates, e.g., 1.0.3:

```txt
pip install 'easytemplates==1.0.3'
```

To upgrade to the latest version:

```txt
pip install --upgrade easytemplates
```
...
```txt
pip install --upgrade --force-reinstall easytemplates
```

To check the installed version:

```txt
pip show easytemplates
```
...
To install EasyTemplates from the `develop` branch of GitHub, for example:

```txt
pip install git+https://github.com/easyscience/templates@develop
```

To include extra dependencies (e.g., dev):

```txt
pip install 'easytemplates[dev] @ git+https://github.com/easyscience/templates@develop'
```

## How to Run Tutorials

EasyTemplates includes a collection of **Jupyter Notebook examples** that
...
- Add JupyterLab and the Pixi kernel for Jupyter:
  ...
  ```
- Download all the EasyTemplates tutorials to the `tutorials/` directory:
  ```txt
  pixi run easytemplates download-all-tutorials
  ```
...
- Add the virtual environment as a Jupyter kernel:
  ```txt
  python -m ipykernel install --user --name=venv --display-name "EasyTemplates Python kernel"
  ```
- Download all the EasyTemplates tutorials to the `tutorials/` directory:
  ```txt
  python -m easytemplates download-all-tutorials
  ```
...
- Open one of the `*.ipynb` files and select the
  `EasyTemplates Python kernel` to get started.
